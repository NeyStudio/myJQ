// 1. Enregistrement du Service Worker (pour la Progressive Web App - PWA)
...
// Notification dans script.js ou ailleurs dans ton HTML
function showMyNotification() {
    ...
    if (Notification.permission === 'granted') {
        new Notification('Titre de ma notification', { body: 'Ceci est le corps de ma notification.' });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const openShopModalBtn = document.getElementById('open-shop-modal-btn'); // NOUVEAU
    const coinsDisplay = document.getElementById('coins-display'); // NOUVEAU
    const gemsDisplay = document.getElementById('gems-display'); // NOUVEAU
    const userNameDisplay = document.getElementById('user-name-display'); // NOUVEAU

    // Références aux modales
    const profileModal = document.getElementById('profile-modal');
    const questFormModal = document.getElementById('quest-form-modal');
    const filtersModal = document.getElementById('filters-modal');
    const shopModal = document.getElementById('shop-modal'); // NOUVEAU
    ...
    const profilePictureInput = document.getElementById('profile-picture-input'); // NOUVEAU
    const userNameInput = document.getElementById('user-name-input'); // NOUVEAU
    const saveProfileBtn = document.getElementById('save-profile-btn'); // NOUVEAU
    ...
    const questCategorySelect = document.getElementById('quest-category'); // NOUVEAU
    ...
    const filterCategorySelect = document.getElementById('filter-category'); // NOUVEAU
    const sortOrderSelect = document.getElementById('sort-order');

    const closeShopModalBtn = document.getElementById('close-shop-modal-btn'); // NOUVEAU
    const shopItemsContainer = shopModal.querySelector('.modal-content'); // NOUVEAU
    ...
    // Thème
    const themeSwitch = document.getElementById('checkbox'); // NOUVEAU
    ...
    let userName = localStorage.getItem('userName') || 'Aventurier Anonyme'; // NOUVEAU
    ...
    let currentFilterCategory = localStorage.getItem('currentFilterCategory') || 'all'; // NOUVEAU
    let currentSortOrder = localStorage.getItem('currentSortOrder') || 'none';
    let editingQuestId = null;
    let isDarkTheme = localStorage.getItem('isDarkTheme') === 'true'; // NOUVEAU
    ...
    // NOUVEAU : Demander la permission de notification au chargement de la page
    if ('Notification' in window) {
        Notification.requestPermission().then(permission => {
            if (permission === 'granted') {
    ...
    function showAlert(title, message, isConfirm = false) {
        return new Promise(resolve => {
            ...
            if (isConfirm) {
                ...
                confirmBtn.className = 'custom-alert-confirm-btn main-button'; // Ajout des classes de style
                customAlertModal.querySelector('.modal-content').appendChild(confirmBtn);

                cancelBtn = document.createElement('button');
                cancelBtn.textContent = 'Non';
                cancelBtn.className = 'custom-alert-cancel-btn secondary-button'; // Ajout des classes de style
    ...
    // NOUVEAU : Fonction pour sauvegarder les infos de profil
    function saveProfile() {
        ...
        modalProfilePicture.src = userProfilePic;
        userNameDisplay.textContent = userName;
        userNameInput.value = userName; // Mettre à jour l'input aussi
        showAlert('Profil Mis à Jour', 'Votre profil a été mis à jour avec succès !');
        saveAndRenderAll(false); // Sauvegarder sans forcer le rendu complet si ce n'est pas nécessaire (ici, juste le profil)
    }

    // Fonction pour ajouter une nouvelle quête
    function addQuest(titre, description, dateOuverture, dateFermeture, difficulte, categorie) { // AJOUT 'categorie'
        const newQuest = {
    ...
    function updateQuest(id, titre, description, dateOuverture, dateFermeture, difficulte, categorie) { // AJOUT 'categorie'
        ...
        if (index !== -1) {
            ...
            quetes[index].categorie = categorie; // NOUVEAU
    ...
    function saveAndRenderAll(fullRender = true) {
        try {
            ...
            localStorage.setItem('currentFilterCategory', currentFilterCategory); // NOUVEAU
            localStorage.setItem('currentSortOrder', currentSortOrder);
            localStorage.setItem('isDarkTheme', isDarkTheme.toString()); // NOUVEAU
            ...
            if (fullRender) {
                ...
                updateLevelBadge(); // Juste mettre à jour le badge XP/Niveau si pas de rendu complet
            }
        } catch (e) {
    ...
    function renderAll() {
        ...
        filterCategorySelect.value = currentFilterCategory; // NOUVEAU
        sortOrderSelect.value = currentSortOrder;
        coinsDisplay.textContent = userCoins; // Mise à jour des pièces
        gemsDisplay.textContent = userGems; // Mise à jour des gemmes
        renderQuetes();
    }
    ...
    function renderQuetes() {
        ...
        const applyFiltersAndSort = () => {
            ...
            tempQuetes = tempQuetes.filter(quete => {
                ...
                let matchesCategory = true; // NOUVEAU
                if (currentFilterCategory !== 'all' && quete.categorie !== currentFilterCategory) {
                    matchesCategory = false;
                }

                return matchesStatus && matchesDifficulty && matchesCategory; // AJOUT matchesCategory
            });
        ...
        // Attacher les écouteurs d'événements aux boutons nouvellement créés (doit être après la boucle forEach)
    ...
    function updateLevelBadge() {
        ...
        if (currentLevel > lastNotifiedLevel) {
            ...
            showAlert(`Niveau Supérieur !`, `Félicitations, vous êtes passé au Niveau ${currentLevel} ! Vous gagnez ${gainedCoins} pièces !`)
                .then(() => {
                    saveAndRenderAll(false); // Sauvegarder après que l'utilisateur a vu l'alerte
                });
        }
                ...
                if (!quete.terminee) {
                    if (now > closeDate && now - closeDate < oneDay) {
                        if (!localStorage.getItem(`notified_expired_${quete.id}`)) {
                            new Notification('Quête Expirée !', {
                                body: `La quête "${quete.titre}" a expiré le ${closeDate.toLocaleDateString()}.`,
                        ...
                        if (!localStorage.getItem(`notified_deadline_${quete.id}`)) {
                            new Notification('Quête urgente !', {
                                body: `La quête "${quete.titre}" expire bientôt (${closeDate.toLocaleDateString()}) !`,
                                icon: 'images/logo.jpg'
                            });
    ...
    function openQuestFormModal(questId = null) {
        ...
        if (questId) {
            ...
            if (queteToEdit) {
                ...
                questCategorySelect.value = queteToEdit.categorie || 'none'; // NOUVEAU: Pré-remplir la catégorie
            } else {
            ...
            questCategorySelect.value = 'none'; // NOUVEAU: Réinitialiser la catégorie
        }
    }

    // NOUVEAU : Fonction pour ouvrir la modale de la boutique
    ...
    function buyItem(itemType) {
        ...
        if (userCoins >= costCoins && userGems >= costGems) {
            ...
            showAlert('Achat Réussi', successMessage).then(() => {
                saveAndRenderAll(false); // Sauvegarder les monnaies et le profil
            });
        } else {
    ...
    profilePictureContainer.addEventListener('click', () => {
        ...
        userNameInput.value = userName; // S'assurer que l'input affiche le nom actuel
    ...
    // NOUVEAU: Enregistrement du nom d'utilisateur et de la photo
    ...
    profilePictureInput.addEventListener('change', (event) => {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                userProfilePic = e.target.result; // Stocke l'image en Base64
                modalProfilePicture.src = userProfilePic;
                profilePicture.src = userProfilePic; // Met à jour l'image du header aussi
                showAlert('Photo de profil', 'Votre photo de profil a été mise à jour !');
                saveProfile(); // Sauvegarder la nouvelle photo
            };
    ...
    addQuestForm.addEventListener('submit', (e) => {
        ...
        const categorie = questCategorySelect.value; // NOUVEAU
        ...
        if (editingQuestId) {
            updateQuest(editingQuestId, titre, description, dateOuverture, dateFermeture, difficulte, categorie); // AJOUT 'categorie'
        } else {
            addQuest(titre, description, dateOuverture, dateFermeture, difficulte, categorie); // AJOUT 'categorie'
        }
    ...
    applyFiltersBtn.addEventListener('click', () => {
        ...
        currentFilterCategory = filterCategorySelect.value; // NOUVEAU
    ...
    resetFiltersBtn.addEventListener('click', () => {
        ...
        currentFilterCategory = 'all'; // NOUVEAU
        currentSortOrder = 'none';

        filterStatusSelect.value = 'all';
        filterDifficultySelect.value = 'all';
        filterCategorySelect.value = 'all'; // NOUVEAU
    ...
    // NOUVEAU : Gérer le changement de thème
